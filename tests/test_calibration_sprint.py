...
def test_compare_shows_regression_honestly(tmp_path):
    ...
    # Patch load_summary to return controlled data. output_path is redirected to
    # tmp_path -- compare() must NEVER overwrite the real project data/ folder
    # with fixture numbers just because a test ran.
    with patch("compare_calibration_runs.load_summary") as mock_ls:
        mock_ls.side_effect = lambda tag: baseline if tag == "base" else post
        result = compare("base", "post", output_path=tmp_path / "report.json")
    assert len(result.get("regressed", [])) > 0
    assert "NO IMPROVEMENT" in result.get("honest_assessment", "")
    assert (tmp_path / "report.json").exists()


def test_compare_no_fake_improvement(tmp_path):
...
def test_compare_does_not_write_to_real_data_dir(tmp_path):
    """Regression test: compare() must not silently pollute the real project
    data/calibration_before_after_report.json with fixture/mocked numbers just
    because a test called it -- this previously made a fake 'no improvement'
    result look like a genuine calibration comparison had been run.
    """
    from compare_calibration_runs import DATA
    compare, _ = _import_compare()
    real_report = DATA / "calibration_before_after_report.json"
    before = real_report.read_text(encoding="utf-8") if real_report.exists() else None

    same = {"decision_match_pct": 1.0, "direction_match_pct": 1.0,
            "hold_rate_pct": 1.0, "avg_return_error_pp": 1.0, "valid_runs": 1}
    with patch("compare_calibration_runs.load_summary") as mock_ls:
        mock_ls.side_effect = lambda tag: same
        compare("base", "post", output_path=tmp_path / "report.json")

    after = real_report.read_text(encoding="utf-8") if real_report.exists() else None
    assert before == after, "compare() must not touch the real data/ report file when given output_path"
